...
build_right_prompt () {
	local index=1 
	local element
	for element in "${POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS[@]}"
	do
		element=${element%_joined} 
		if [[ $element[0,7] =~ "custom_" ]]
		then
...
exit_code_or_status () {
	local ec=$1 
	if [[ "$POWERLEVEL9K_STATUS_HIDE_SIGNAME" = true ]]
	then
		echo "$ec"
	elif (( ec <= 128 ))
	then
...
getColorCode () {
	...
	then
		echo -n 'none'
	elif [[ "${colorName}" == "foreground" ]]
	then
		for i in "${(k@)__P9K_COLORS}"
		do
			print -P "$(foregroundColor $i)$(getColor $i) - $i%f"
		done
	elif [[ "${colorName}" == "background" ]]
	then
		for i in "${(k@)__P9K_COLORS}"
		do
...
left_prompt_segment () {
	...
	if [[ $CURRENT_BG != 'NONE' ]] && ! isSameColor "${backgroundColor}" "$CURRENT_BG"
	then
	...
	elif isSameColor "$CURRENT_BG" "${backgroundColor}"
	then
		local complement
...
p9k::parseIp () {
	...
	then
		...
		for rawInterface in $interfaces
		do
			[[ "$rawInterface" =~ $pattern ]] && relevantInterfaces+=($MATCH) 
		done
		local newline=$'\n' 
		for interfaceName in $relevantInterfaces
		do
...
$(print_icon 'MULTILINE_LAST_PROMPT_PREFIX')' 
		if [[ "$POWERLEVEL9K_RPROMPT_ON_NEWLINE" != true ]]
		then
	...
	if [[ "$POWERLEVEL9K_DISABLE_RPROMPT" != true ]]
	then
		RPROMPT="${RPROMPT_PREFIX}"'%f%b%k$(build_right_prompt)%{$reset_color%}'"${RPROMPT_SUFFIX}" 
	fi
	local NEWLINE='
' 
	if [[ $POWERLEVEL9K_PROMPT_ADD_NEWLINE == true ]]
	then
		NEWLINES="" 
		repeat ${POWERLEVEL9K_PROMPT_ADD_NEWLINE_COUNT:-1}
		do
			NEWLINES+=$NEWLINE 
		done
...
powerlevel9k_vcs_init () {
	if [[ -n "$POWERLEVEL9K_CHANGESET_HASH_LENGTH" ]]
	then
	...
	vcs_states=('clean' 'green' 'modified' 'yellow' 'untracked' 'green') 
	VCS_CHANGESET_PREFIX='' 
	if [[ "$POWERLEVEL9K_SHOW_CHANGESET" == true ]]
	then
...
print_deprecation_warning () {
	...
	for key in ${(@k)raw_deprecated_segments}
	do
		if segment_in_use $key
		then
...
prompt_background_jobs () {
	...
	if [[ background_jobs_number -gt 0 ]]
	then
		local background_jobs_number_print="" 
		if [[ "$POWERLEVEL9K_BACKGROUND_JOBS_VERBOSE" == "true" ]] && (
				[[ "$background_jobs_number" -gt 1 ]] || [[ "$POWERLEVEL9K_BACKGROUND_JOBS_VERBOSE_ALWAYS" == "true" ]]
			)
		then
...
prompt_battery () {
	...
	battery_states=('low' 'red' 'charging' 'yellow' 'charged' 'green' 'disconnected' "$DEFAULT_COLOR_INVERTED") 
	local ROOT_PREFIX="${4}" 
	set_default POWERLEVEL9K_BATTERY_LOW_THRESHOLD 10
	if [[ $OS =~ OSX && -f "${ROOT_PREFIX}"/usr/bin/pmset && -x "${ROOT_PREFIX}"/usr/bin/pmset ]]
	then
		...
		case $(echo $raw_data | awk -F ';' '{print $2}' | awk '{$1=$1};1') in
			('charging' | 'finishing charge' | 'AC attached') current_state="charging" 
				remain=" ($tstring)"  ;;
			('discharging') [[ $bat_percent -lt $POWERLEVEL9K_BATTERY_LOW_THRESHOLD ]] && current_state="low"  || current_state="disconnected" 
				remain=" ($tstring)"  ;;
	...
	then
		...
		if [[ -f ${ROOT_PREFIX}/usr/bin/acpi ]]
		then
			local time_remaining=$(${ROOT_PREFIX}/usr/bin/acpi | awk '{ print $5 }') 
			if [[ $time_remaining =~ rate ]]
			then
				local tstring="..." 
			elif [[ $time_remaining =~ "[[:digit:]]+" ]]
			then
	...
	if [[ "$POWERLEVEL9K_BATTERY_VERBOSE" == true ]]
	then
		message="$bat_percent%%$remain" 
	else
		message="$bat_percent%%" 
	fi
	if [[ -n "$POWERLEVEL9K_BATTERY_STAGES" ]]
	then
	...
	if defined POWERLEVEL9K_BATTERY_HIDE_ABOVE_THRESHOLD && [[ "${bat_percent}" -ge $POWERLEVEL9K_BATTERY_HIDE_ABOVE_THRESHOLD ]]
	then
		return
	fi
	if [[ -n "$POWERLEVEL9K_BATTERY_LEVEL_BACKGROUND" ]] && [[ "${(t)POWERLEVEL9K_BATTERY_LEVEL_BACKGROUND}" =~ "array" ]]
	then
...
prompt_chruby () {
	...
	local chruby_label="" 
	if [[ "$POWERLEVEL9K_CHRUBY_SHOW_ENGINE" == true ]]
	then
		chruby_label+="$RUBY_ENGINE " 
	fi
	if [[ "$POWERLEVEL9K_CHRUBY_SHOW_VERSION" == true ]]
	then
...
prompt_command_execution_time () {
	...
	if (( _P9K_COMMAND_DURATION >= POWERLEVEL9K_COMMAND_EXECUTION_TIME_THRESHOLD ))
	then
...
prompt_context () {
	...
	context_states=("ROOT" "yellow" "SUDO" "yellow" "DEFAULT" "yellow" "REMOTE" "yellow" "REMOTE_SUDO" "yellow") 
	local content="" 
	if [[ "$POWERLEVEL9K_ALWAYS_SHOW_CONTEXT" == true ]] || [[ "$(whoami)" != "$DEFAULT_USER" ]] || [[ -n "$SSH_CLIENT" || -n "$SSH_TTY" ]]
	then
		content="${POWERLEVEL9K_CONTEXT_TEMPLATE}" 
	elif [[ "$POWERLEVEL9K_ALWAYS_SHOW_USER" == true ]]
	then
		content="$(whoami)" 
	else
		return
...
prompt_dir () {
	...
	if [[ -n "$POWERLEVEL9K_SHORTEN_DIR_LENGTH" || "$POWERLEVEL9K_SHORTEN_STRATEGY" == "truncate_with_folder_marker" || "$POWERLEVEL9K_SHORTEN_STRATEGY" == "truncate_to_last" || "$POWERLEVEL9K_SHORTEN_STRATEGY" == "truncate_with_package_name" ]]
	then
		set_default POWERLEVEL9K_SHORTEN_DELIMITER "\u2026"
		local delim=$(echo -n $POWERLEVEL9K_SHORTEN_DELIMITER) 
		case "$POWERLEVEL9K_SHORTEN_STRATEGY" in
			(truncate_absolute_chars) if [ ${#current_path} -gt $(( $POWERLEVEL9K_SHORTEN_DIR_LENGTH + ${#POWERLEVEL9K_SHORTEN_DELIMITER} )) ]
				then
					current_path=$POWERLEVEL9K_SHORTEN_DELIMITER${current_path:(-POWERLEVEL9K_SHORTEN_DIR_LENGTH)} 
				fi ;;
			(truncate_middle) current_path=$(truncatePath $current_path $POWERLEVEL9K_SHORTEN_DIR_LENGTH $POWERLEVEL9K_SHORTEN_DELIMITER "middle")  ;;
			(truncate_from_right) current_path=$(truncatePath "$current_path" $POWERLEVEL9K_SHORTEN_DIR_LENGTH $POWERLEVEL9K_SHORTEN_DELIMITER)  ;;
			(truncate_absolute) if [ ${#current_path} -gt $(( $POWERLEVEL9K_SHORTEN_DIR_LENGTH + ${#POWERLEVEL9K_SHORTEN_DELIMITER} )) ]
				then
					current_path=$POWERLEVEL9K_SHORTEN_DELIMITER${current_path:(-POWERLEVEL9K_SHORTEN_DIR_LENGTH)} 
				fi ;;
			(truncate_to_last) current_path=${current_path##*/}  ;;
			(truncate_to_first_and_last) if (( ${#current_path} > 1 )) && (( ${POWERLEVEL9K_SHORTEN_DIR_LENGTH} > 0 ))
				then
			...
			(truncate_to_unique) if (( ${#current_path} > 1 ))
				then
			...
			(truncate_with_folder_marker) if (( ${#paths} > 0 ))
				then
					local last_marked_folder marked_folder
					set_default POWERLEVEL9K_SHORTEN_FOLDER_MARKER ".shorten_folder_marker"
					for marked_folder in $(upsearch $POWERLEVEL9K_SHORTEN_FOLDER_MARKER)
					do
						if [[ "$marked_folder" == "/" ]]
						then
							trunc_path="/" 
						elif [[ "$marked_folder" == "$HOME" ]]
						then
							trunc_path="~" 
						elif [[ "${marked_folder%/*}" == $last_marked_folder ]]
						then
			...
			(truncate_with_package_name) local name repo_path package_path current_dir zero
				...
				elif [[ $(git rev-parse --is-inside-git-dir 2> /dev/null) == "true" ]]
				then
				...
				for file in "${POWERLEVEL9K_DIR_PACKAGE_FILES[@]}"
				do
					if [[ -f "${package_path}/${file}" ]]
					then
	...
	dir_states=("DEFAULT" "FOLDER_ICON" "HOME" "HOME_ICON" "HOME_SUBFOLDER" "HOME_SUB_ICON" "NOT_WRITABLE" "LOCK_ICON" "ETC" "ETC_ICON") 
	local state_path="$(print -P '%~')" 
	local current_state="DEFAULT" 
	if [[ $state_path == '/etc'* ]]
	then
		current_state='ETC' 
	elif [[ "${POWERLEVEL9K_DIR_SHOW_WRITABLE}" == true && ! -w "$PWD" ]]
	then
	...
	if [[ "${(L)POWERLEVEL9K_DIR_PATH_HIGHLIGHT_BOLD}" == "true" ]]
	then
		bld_on="%B" 
		bld_off="%b" 
	else
	...
	dir_name=${path_opt%/*} 
	base_name=${path_opt##*/} 
	if [[ -n ${POWERLEVEL9K_DIR_PATH_HIGHLIGHT_FOREGROUND} ]]
	then
		if [[ $path_opt == "/" || $path_opt == "~" ]]
		then
	...
	if [[ "${POWERLEVEL9K_DIR_OMIT_FIRST_CHARACTER}" == "true" && $path_opt != "/" && $path_opt != "~" ]]
	then
		current_path="${current_path[2,-1]}" 
	fi
	if [[ -n ${POWERLEVEL9K_DIR_PATH_SEPARATOR_FOREGROUND} && $path_opt != "/" ]]
	then
		local repl="%F{$POWERLEVEL9K_DIR_PATH_SEPARATOR_FOREGROUND}/%F{$dir_state_foreground}" 
		current_path=${current_path//\//$repl} 
	fi
	if [[ "${POWERLEVEL9K_DIR_PATH_SEPARATOR}" != "/" && $path_opt != "/" ]]
	then
		current_path=${current_path//\//$POWERLEVEL9K_DIR_PATH_SEPARATOR} 
	fi
	if [[ "${POWERLEVEL9K_HOME_FOLDER_ABBREVIATION}" != "~" && ! "${(L)POWERLEVEL9K_DIR_OMIT_FIRST_CHARACTER}" == "true" ]]
	then
...
prompt_disk_usage () {
	...
	hdd_usage_forecolors=('normal' 'yellow' 'warning' "$DEFAULT_COLOR" 'critical' 'white') 
	typeset -AH hdd_usage_backcolors
	hdd_usage_backcolors=('normal' $DEFAULT_COLOR 'warning' 'yellow' 'critical' 'red') 
	local disk_usage="${$(\df -P . | sed -n '2p' | awk '{ print $5 }')%%\%}" 
	if [ "$disk_usage" -ge "$POWERLEVEL9K_DISK_USAGE_WARNING_LEVEL" ]
	then
		current_state='warning' 
		if [ "$disk_usage" -ge "$POWERLEVEL9K_DISK_USAGE_CRITICAL_LEVEL" ]
		then
			current_state='critical' 
		fi
	else
		if [[ "$POWERLEVEL9K_DISK_USAGE_ONLY_WARNING" == true ]]
		then
			current_state='' 
			return
		fi
...
prompt_go_version () {
	...
	if [[ -n "$go_version" && "${PWD##$go_path}" != "$PWD" ]]
	then
...
prompt_host () {
	...
	then
		host_state=("STATE" "REMOTE" "CONTENT" "${POWERLEVEL9K_HOST_TEMPLATE}" "BACKGROUND_COLOR" "${DEFAULT_COLOR}" "FOREGROUND_COLOR" "yellow" "VISUAL_IDENTIFIER" "SSH_ICON") 
	else
		host_state=("STATE" "LOCAL" "CONTENT" "${POWERLEVEL9K_HOST_TEMPLATE}" "BACKGROUND_COLOR" "${DEFAULT_COLOR}" "FOREGROUND_COLOR" "yellow" "VISUAL_IDENTIFIER" "HOST_ICON") 
	fi
...
prompt_laravel_version () {
	...
	if [[ -n "${laravel_version}" && "${laravel_version}" =~ "Laravel Framework" ]]
	then
...
prompt_load () {
	...
	load_states=('critical' 'red' 'warning' 'yellow' 'normal' 'green') 
	...
	case "$OS" in
		(OSX | BSD) load_avg=$(sysctl vm.loadavg | grep -o -E '[0-9]+(\.|,)[0-9]+' | sed -n ${load_select}p) 
			if [[ "$OS" == "OSX" ]]
			then
	...
	load_avg=${load_avg//,/.} 
	if [[ "$load_avg" -gt $((${cores} * 0.7)) ]]
	then
		current_state="critical" 
	elif [[ "$load_avg" -gt $((${cores} * 0.5)) ]]
	then
		current_state="warning" 
	else
		current_state="normal" 
...
prompt_powerlevel9k_setup () {
	...
	termColors
	if [[ $POWERLEVEL9K_IGNORE_TERM_LANG == false ]]
	then
		local term_lang
		term_lang=$(echo $LANG) 
		if [[ $term_lang == 'C' ]]
	...
	deprecated_segments=('longstatus' 'status') 
	print_deprecation_warning deprecated_segments
	autoload -U colors && colors
	if segment_in_use "vcs"
	then
		powerlevel9k_vcs_init
	fi
...
prompt_public_ip () {
	...
	if [[ -f $POWERLEVEL9K_PUBLIC_IP_FILE ]]
	then
		typeset -i timediff
		if [[ "$OS" == "OSX" ]]
		then
	...
	local fresh_ip
	if [[ $refresh_ip == true && -w $POWERLEVEL9K_PUBLIC_IP_FILE ]]
	then
		for method in "${POWERLEVEL9K_PUBLIC_IP_METHODS[@]}"
		do
			case $method in
				('dig') fresh_ip="$(dig +time=1 +tries=1 +short myip.opendns.com @resolver1.opendns.com 2> /dev/null)" 
					[[ "$fresh_ip" =~ ^\; ]] && unset fresh_ip ;;
				('curl') fresh_ip="$(curl --max-time 10 -w '\n' "$POWERLEVEL9K_PUBLIC_IP_HOST" 2> /dev/null)"  ;;
				('wget') fresh_ip="$(wget -T 10 -qO- "$POWERLEVEL9K_PUBLIC_IP_HOST" 2> /dev/null)"  ;;
			esac
			if [[ -n "${fresh_ip}" ]]
			then
				break
	...
	then
		icon='PUBLIC_IP_ICON' 
		if [[ -n $POWERLEVEL9K_PUBLIC_IP_VPN_INTERFACE ]]
		then
...
prompt_pyenv () {
	...
	then
		...
		if [[ -f "${pyenv_root}/version" ]]
		then
			pyenv_global="$(pyenv version-file-read ${pyenv_root}/version)" 
		fi
		if [[ "${pyenv_version_name}" != "${pyenv_global}" || "${POWERLEVEL9K_PYENV_PROMPT_ALWAYS_SHOW}" == "true" ]]
		then
...
prompt_rbenv () {
	...
	then
		...
		if [[ "${rbenv_version_name}" != "${rbenv_global}" || "${POWERLEVEL9K_RBENV_PROMPT_ALWAYS_SHOW}" == "true" ]]
		then
...
prompt_status () {
	...
	local ec_sum
	local ec
	if [[ $POWERLEVEL9K_STATUS_SHOW_PIPESTATUS == true ]]
	then
		if (( $#RETVALS > 1 ))
		then
	...
	if (( ec_sum > 0 ))
	then
		if [[ "$POWERLEVEL9K_STATUS_CROSS" == false && "$POWERLEVEL9K_STATUS_VERBOSE" == true ]]
		then
			"$1_prompt_segment" "$0_ERROR" "$2" "red" "yellow1" "$ec_text" 'CARRIAGE_RETURN_ICON'
		else
			"$1_prompt_segment" "$0_ERROR" "$2" "$DEFAULT_COLOR" "red" "" 'FAIL_ICON'
		fi
	elif [[ "$POWERLEVEL9K_STATUS_OK" == true ]] && [[ "$POWERLEVEL9K_STATUS_VERBOSE" == true || "$POWERLEVEL9K_STATUS_OK_IN_NON_VERBOSE" == true ]]
	then
...
prompt_user () {
	...
	if [[ "$POWERLEVEL9K_ALWAYS_SHOW_USER" == true ]] || [[ "$(whoami)" != "$DEFAULT_USER" ]]
	then
		if [[ $(print -P "%#") == '#' ]]
		then
			user_state=("STATE" "ROOT" "CONTENT" "${POWERLEVEL9K_USER_TEMPLATE}" "BACKGROUND_COLOR" "${DEFAULT_COLOR}" "FOREGROUND_COLOR" "yellow" "VISUAL_IDENTIFIER" "ROOT_ICON") 
		elif [[ -n "$SUDO_COMMAND" ]]
		then
			user_state=("STATE" "SUDO" "CONTENT" "${POWERLEVEL9K_USER_TEMPLATE}" "BACKGROUND_COLOR" "${DEFAULT_COLOR}" "FOREGROUND_COLOR" "yellow" "VISUAL_IDENTIFIER" "SUDO_ICON") 
		else
			user_state=("STATE" "DEFAULT" "CONTENT" "$(whoami)" "BACKGROUND_COLOR" "${DEFAULT_COLOR}" "FOREGROUND_COLOR" "yellow" "VISUAL_IDENTIFIER" "USER_ICON") 
		fi
...
prompt_vi_mode () {
	case ${KEYMAP} in
		(vicmd) "$1_prompt_segment" "$0_NORMAL" "$2" "$DEFAULT_COLOR" "white" "$POWERLEVEL9K_VI_COMMAND_MODE_STRING" ;;
...
right_prompt_segment () {
	...
	[[ -n "${backgroundColor}" ]] && background="$(backgroundColor ${backgroundColor})"  || background="%k" 
	[[ -n "${foregroundColor}" ]] && foreground="$(foregroundColor ${foregroundColor})"  || foreground="%f" 
	if [[ "$CURRENT_RIGHT_BG" != "NONE" ]]
	then
		echo -n "${POWERLEVEL9K_WHITESPACE_BETWEEN_RIGHT_SEGMENTS}%f"
	fi
	if [[ $joined == false ]] || [[ "$CURRENT_RIGHT_BG" == "NONE" ]]
	then
		if isSameColor "$CURRENT_RIGHT_BG" "${backgroundColor}"
		then
			local complement
			[[ -n "${foregroundColor}" ]] && complement="${foreground}"  || complement="$(foregroundColor $DEFAULT_COLOR)" 
...
segmentShouldBeJoined () {
	...
	local joined=false 
	if [[ ${current_segment[-7,-1]} == '_joined' ]]
	then
		joined=true 
		if (( $(($current_index - $last_segment_index)) > 1))
		then
			local examined_index=$((current_index - 1)) 
			while (( $examined_index > $last_segment_index ))
			do
				local previous_segment=${elements[$examined_index]} 
				if [[ ${previous_segment[-7,-1]} != '_joined' ]]
				then
					joined=false 
					break
				fi
...
segment_in_use () {
	local key=$1 
	[[ -n "${POWERLEVEL9K_LEFT_PROMPT_ELEMENTS[(r)${key}]}" || -n "${POWERLEVEL9K_LEFT_PROMPT_ELEMENTS[(r)${key}_joined]}" || -n "${POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS[(r)${key}]}" || -n "${POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS[(r)${key}_joined]}" ]]
}
set_default () {
	local varname="$1" 
...
termColors () {
	if [[ $POWERLEVEL9K_IGNORE_TERM_COLORS == true ]]
	then
		return
	fi
	local term_colors
...
truncatePath () {
	...
	then
		...
		case $4 in
			(right) local threshhold=$(( $2 + ${#3} )) 
				for ((i=1; i<${#paths}; i++ )) do
					test_dir=$paths[$i] 
					test_dir_length=${#test_dir} 
					if (( $test_dir_length > $threshhold )) && (( $test_dir_length > 3 ))
					then
			...
			(middle) local threshhold=$(( $2 * 2 )) 
				local last_pos
				for ((i=1; i<${#paths}; i++ )) do
					test_dir=$paths[$i] 
					test_dir_length=${#test_dir} 
					if (( $test_dir_length > $threshhold ))
					then
...
truncatePathFromRight () {
	...
	echo $1 | sed $SED_EXTENDED_REGEX_PARAMETER "s@(([^/]{$((POWERLEVEL9K_SHORTEN_DIR_LENGTH))})([^/]{$delim_len}))[^/]+/@\2$POWERLEVEL9K_SHORTEN_DELIMITER/@g"
}
try_alias_value () {
...
alias -- 9='cd -9'
alias -- _='sudo '
alias -- claude=/Users/tanghehui/.claude/local/claude
alias -- egrep='grep -E'
alias -- fgrep='grep -F'
alias -- g=git
alias -- ga='git add'
...
alias -- rd=rmdir
alias -- run-help=man
alias -- show-claude-usage='claude-monitor --timezone Asia/Shanghai --plan max5'
alias -- which-command=whence
# Check for rg availability
if ! command -v rg >/dev/null 2>&1; then
  alias rg='/Users/tanghehui/anaconda3/bin/rg'
fi
export PATH='/Users/tanghehui/.nvm/versions/node/v22.2.0/bin:/Users/tanghehui/bin:/opt/homebrew/bin:/usr/local/bin:/usr/local/bin:/usr/local/sbin:/opt/homebrew/bin:/opt/homebrew/sbin:/usr/local/bin:/System/Cryptexes/App/usr/bin:/usr/bin:/bin:/usr/sbin:/sbin:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/local/bin:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/bin:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/appleinternal/bin:/Library/Apple/usr/bin:/Users/tanghehui/anaconda3/bin:/Users/tanghehui/anaconda3/condabin:/Users/tanghehui/.nvm/versions/node/v22.2.0/bin:/Users/tanghehui/bin:/usr/local/sbin:/Users/tanghehui/Library/Application Support/JetBrains/Toolbox/scripts:/Users/tanghehui/.vscode/extensions/ms-python.debugpy-2025.8.0-darwin-arm64/bundled/scripts/noConfigScripts:/Users/tanghehui/Library/Application Support/JetBrains/Toolbox/scripts'
